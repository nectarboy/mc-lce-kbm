...
void UIController::handleKeyPress(unsigned int iPad, unsigned int key)
{
	...
	down = InputManager.ButtonDown(iPad,key) | PCInputManager.ButtonDown(iPad, key);
	pressed = InputManager.ButtonPressed(iPad,key) | PCInputManager.ButtonPressed(iPad, key); // Toggle
	released = InputManager.ButtonReleased(iPad,key) | PCInputManager.ButtonReleased(iPad, key); // Toggle
	...
	if(bVal)
	{
		if(iPad==XUSER_INDEX_ANY)
		{
			for(int i=0;i<XUSER_MAX_COUNT;i++)
			{
				...
				PCInputManager.SetMenuDisplayed(i,true);
		...
		else
		{
			...
			PCInputManager.SetMenuDisplayed(iPad,true);
	...
	for(int iPad=0;iPad<XUSER_MAX_COUNT;iPad++)
	{
		if(m_bMenuToBeClosed[iPad])
		{
			...
			else
			{
				...
				PCInputManager.SetMenuDisplayed(iPad,false);
			}
