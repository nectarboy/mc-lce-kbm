		...
		//while (running)
		{
			...
			for( int i = 0; i < XUSER_MAX_COUNT; i++ )
			{
					...
					if(localplayers[i])
				{
					...
					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_JUMP))				localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_JUMP;
					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_USE))					localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_USE;

					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_INVENTORY))				localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_INVENTORY;
					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_ACTION))					localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_ACTION;
					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_CRAFTING))				localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_CRAFTING;
					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_PAUSEMENU))
					{
...
#endif
					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_DROP))					localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_DROP;

					// 4J-PB - If we're flying, the sneak needs to be held on to go down
					if(localplayers[i]->abilities.flying)
					{
						if(PCInputManager.ButtonDown(i, MINECRAFT_ACTION_SNEAK_TOGGLE))			localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_SNEAK_TOGGLE;
					}
					else
					{
						if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_SNEAK_TOGGLE))		localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_SNEAK_TOGGLE;
					}
					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_RENDER_THIRD_PERSON))		localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_RENDER_THIRD_PERSON;
					if(PCInputManager.ButtonPressed(i, MINECRAFT_ACTION_GAME_INFO))				localplayers[i]->ullButtonsPressed|=1LL<<MINECRAFT_ACTION_GAME_INFO;
...
#ifndef __PSVITA__
						if(PCInputManager.ButtonDown(i, MINECRAFT_ACTION_DPAD_LEFT))					{ localplayers[i]->ullDpad_this|=1LL<<MINECRAFT_ACTION_DPAD_LEFT;		dirCount++; }
						if(PCInputManager.ButtonDown(i, MINECRAFT_ACTION_DPAD_RIGHT))					{ localplayers[i]->ullDpad_this|=1LL<<MINECRAFT_ACTION_DPAD_RIGHT;		dirCount++; }
						if(PCInputManager.ButtonDown(i, MINECRAFT_ACTION_DPAD_UP))					{ localplayers[i]->ullDpad_this|=1LL<<MINECRAFT_ACTION_DPAD_UP;			dirCount++; }
						if(PCInputManager.ButtonDown(i, MINECRAFT_ACTION_DPAD_DOWN))					{ localplayers[i]->ullDpad_this|=1LL<<MINECRAFT_ACTION_DPAD_DOWN;		dirCount++; }
#endif
	...
	*/
	...
	if (screen == NULL && !ui.GetMenuDisplayed(iPad) )
	{
		...
		*piAction=-1;
		...
		wheel += PCInputManager.GetCoarseWheelDelta(iPad);
		if (wheel != 0)
		{
		...
		if( gameMode->isInputAllowed(MINECRAFT_ACTION_ACTION) )
		{
			...
			if ((InputManager.ButtonDown(iPad, MINECRAFT_ACTION_ACTION) || PCInputManager.ButtonDown(iPad, MINECRAFT_ACTION_ACTION)) && ticks - player->lastClickTick[0] >= timer->ticksPerSecond / 4)
			{
			...
			if(InputManager.ButtonDown(iPad, MINECRAFT_ACTION_ACTION) || PCInputManager.ButtonDown(iPad, MINECRAFT_ACTION_ACTION))
			{
		...
		*/
		...
		{
			if(!(InputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE) || PCInputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE))) gameMode->releaseUsingItem(player);
		}
		else if( gameMode->isInputAllowed(MINECRAFT_ACTION_USE) )
		{
			if( player->abilities.instabuild )
			{
				...
				bool didClick = player->creativeModeHandleMouseClick(1, InputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE) | PCInputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE) );
				...
				if( player->lastClickState == LocalPlayer::lastClick_oldRepeat )
				{
					...
					else
					{
						...
						if ((InputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE) || PCInputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE)) && ticks - player->lastClickTick[1] >= timer->ticksPerSecond / 4)
						{
			...
			else
			{
				...
				if (InputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE) || PCInputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE) )
				{
	...
	{
		// 4J-PB
		if ((InputManager.GetValue(iPad, ACTION_MENU_CANCEL) > 0 || PCInputManager.ButtonDown(iPad, ACTION_MENU_CANCEL)) && gameMode->isInputAllowed(ACTION_MENU_CANCEL))
		{
			setScreen(NULL);
		}
	}
