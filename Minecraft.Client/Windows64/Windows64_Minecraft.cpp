...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	{
	case WM_LBUTTONDOWN:
		PCInputManager.SetMouseState(0, 0, PCInput::PressedState);
		break;
	case WM_LBUTTONUP:
		PCInputManager.SetMouseState(0, 0, PCInput::ReleasedState);
		break;
	case WM_RBUTTONDOWN:
		PCInputManager.SetMouseState(0, 1, PCInput::PressedState);
		break;
	case WM_RBUTTONUP:
		PCInputManager.SetMouseState(0, 1, PCInput::ReleasedState);
		break;
	case WM_INPUT:
	{
		...
		if (raw->header.dwType == RIM_TYPEMOUSE) {
			if (raw->data.mouse.usButtonFlags & RI_MOUSE_WHEEL) {
				float wheelDelta = (float)(short)raw->data.mouse.usButtonData / WHEEL_DELTA;
				//app.DebugPrintf("Yay! %f \n", wheelDelta);
				PCInputManager.AddWheelDelta(0, wheelDelta);
			}
			else {
				PCInputManager.AddMouseDelta(0, raw->data.mouse.lLastX, raw->data.mouse.lLastY);
				//if (PCInputManager.GetMouseDeltaX(0) != 0) {
					// app.DebugPrintf("Yay! %f $\n", PCInputManager.GetMouseDeltaX(0));
				//}
			}
		}
		break;
	...
	case WM_MOUSEMOVE:
		PCInputManager.SetMouseCoords(0, GET_X_LPARAM(lParam), GET_Y_LPARAM(lParam));
		break;
	case WM_KEYDOWN:
		PCInputManager.SetKeyState(0, wParam, PCInput::PressedState);
		// app.DebugPrintf("KeyDown! %d \n", PCInputManager.ButtonDown(0, ACTION_MENU_OK));
		break;
	case WM_KEYUP:
		PCInputManager.SetKeyState(0, wParam, PCInput::ReleasedState);
		break;
	case WM_COMMAND:
		wmId    = LOWORD(wParam);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	g_rid.usUsagePage = 0x01; 
	g_rid.usUsage = 0x02; 
	g_rid.dwFlags = 0;  // input only when active window
	g_rid.hwndTarget = g_hWnd; // Your main window handle
	if (!RegisterRawInputDevices(&g_rid, 1, sizeof(RAWINPUTDEVICE))) {
		app.DebugPrintf("Raw Input Device registration failed with error: %lu\n", GetLastError());
	}
...
#pragma message(__LOC__"Need to define the _FINAL_BUILD before submission")
	...
	while( WM_QUIT != msg.message )
	{
		...
		PCInputManager.Tick();
		PCInputManager.UpdateWindow(&g_hWnd);
	}
