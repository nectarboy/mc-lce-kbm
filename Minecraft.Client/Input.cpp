...
void Input::tick(LocalPlayer *player)
{
	...
	if( pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_LEFT) || pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_RIGHT) )
		xa = -PCInputManager.GetJoypadStick_LX(iPad);
	else
		xa = 0.0f;
	...
	if( pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_FORWARD) || pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_BACKWARD) )
		ya = PCInputManager.GetJoypadStick_LY(iPad);
	else
		ya = 0.0f;
	...
	// Mouse look
	if( pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_LOOK_LEFT) || pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_LOOK_RIGHT) )
		tx = PCInputManager.GetJoypadStick_RX(iPad)*(((float)app.GetGameSettings(iPad,eGameSetting_Sensitivity_InGame))/100.0f); // apply sensitivity to look
	if( pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_LOOK_UP) || pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_LOOK_DOWN) )
		ty = -PCInputManager.GetJoypadStick_RY(iPad)*(((float)app.GetGameSettings(iPad,eGameSetting_Sensitivity_InGame))/100.0f); // apply sensitivity to look

	// Mouse lock
	if (PCInputManager.GetMenuDisplayed(iPad))
	{
		PCInputManager.SetCursorLock(iPad, false);
	}
	else {
		PCInputManager.SetCursorLock(iPad, true);
	}

	float mouseSpeed = 1;
	player->interpolateTurn(tx * mouseSpeed, ty * mouseSpeed);

    //jumping = controller.isButtonPressed(0);

	
	bool jump = PCInputManager.KeyDown(iPad, VK_SPACE);
	if( jump && pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_JUMP) )
		jumping = true;
	else
 		jumping = false;
